...
export interface IValidation {
  rule: (value: string) => boolean | RegExpMatchArray;
  message: string;
}
...
export const email = (value: string): RegExpMatchArray =>
  value.match(validEmail);

const validPassword = /^(?=.*[0-9])(?=.*[A-Z])(?=.*[a-z]).{9,}$/;
export const password = (value: string): RegExpMatchArray =>
  value.match(validPassword);
...
export const findError = (
  ...
  const error = validations.find(
    (validation: IValidation) => !validation.rule(value)
  );
